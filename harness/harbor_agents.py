    ...
    *,
    ...
    script = "\n".join([
        ...
        *run_lines,
        ...
        "  repo=$PWD",
        "elif [[ -d /testbed ]]; then",
        "  git_dir=$(find /testbed -maxdepth 3 -mindepth 2 -type d -name .git -print -quit)",
        "  [[ -z \"$git_dir\" ]] || repo=${git_dir%/.git}",
        "fi",
        "if [[ -n \"$repo\" ]]; then",
        "  git -C \"$repo\" ls-files --others --exclude-standard -z 2>/dev/null | while IFS= read -r -d '' path; do",
        "    git -C \"$repo\" add -N -- \"$path\" 2>/dev/null || true",
        "  done",
        "  git -C \"$repo\" diff HEAD --text > /logs/artifacts/workdir_diff.patch 2>/dev/null || true",
        "  git -C \"$repo\" status --porcelain=v1 > /logs/artifacts/workdir_status.txt 2>/dev/null || true",
        "fi",
        "exit \"$agent_status\"",
    ])
